...
- `testUiShellThemeContract()` — 17 static scenarios covering 240px/72px desktop sidebar states, the 64px utility bar, four approved destinations, absent future modules and SaaS decoration, Light/Dark/System selection, validated browser-local persistence, System fallback, pre-render theme application, exact dark tokens, print-light and Chart.js synchronization, mobile drawer/focus preservation, v1 destination compatibility, single listener initialization, and the 72-ID/two-selector query ceiling.
...
- `96.Tests.Cases.js` contains 34 directly runnable `test*` functions; the ordered runner selects 33 of them plus `getDashboardData()` for its fixed 34-entry gate, while `testInteractiveDrilldownContract()` is covered through `testChartPresentationContract()`.
- `98.Tests.Runner.js` contains only the ordered, fail-fast unified 34-entry suite.
...
Use the individual functions for targeted debugging after `runAllBackendTests()` identifies a failure. The wrapper logs a start marker, one PASS per completed test, and a final `34/34` marker. On failure it logs the test name and error message, then immediately rethrows the original error.
