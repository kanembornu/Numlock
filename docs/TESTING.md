...
- `96.Tests.Cases.js` contains 35 directly runnable `test*` functions; the ordered runner selects 34 of them plus `getDashboardData()` for its fixed 35-entry gate, while `testInteractiveDrilldownContract()` is covered through `testChartPresentationContract()`.
- `98.Tests.Runner.js` contains only the ordered, fail-fast unified 35-entry suite.
...
Use the individual functions for targeted debugging after `runAllBackendTests()` identifies a failure. The wrapper logs a start marker, one PASS per completed test, and a final `35/35` marker. On failure it logs the test name and error message, then immediately rethrows the original error.
...
`runAllBackendTests()` is the unified backend gate for local and Apps Script validation. It requires `35/35`, adding the nine-destination navigation contract to the prior deterministic feature, response, accessibility, UI, performance, and data-quality coverage. The unified suite remains ordered and fail-fast.
