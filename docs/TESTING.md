...
`testSummaryFixtures()` replaces `testSummaryMigration()` in the unified `8/8` suite. It covers mixed sales, purchases, multiple active days, repeated products, distinct quantity and revenue leaders, zero values, and an empty dataset. Its expected Summary fields are hardcoded independently rather than generated by `buildSummary()`. The legacy Summary test remains independently runnable until the fixture test passes live in Apps Script and a separate removal is approved.
