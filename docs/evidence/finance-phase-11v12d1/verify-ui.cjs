...
const dto={id:'PE-fixture-0001',transactionType:'PurchaseEvent',date:'2026-10-01',item:'Sirup Blue Citrus',purchaseQuantity:'1',purchaseUOM:'unknown-package',acquisitionValue:23456, supplier:'TEST',reference:'TEST',isActive:true,financialTotalsIncluded:false,operationalStatus:'Cost recorded',operationalNote:'Cost recorded. Quantity or package is not verified for stock tracking.'};
purchase();ui.submitReceiptEntry();pending.shift().success({success:true,status:'PURCHASE_CAPTURED',data:dto});assert.match(entry.successDetails.innerHTML,/not verified/);
vm.runInContext(read('192.View.Transactions.State.html'),ui);vm.runInContext(read('193.View.Transactions.Render.html'),ui);
const table={innerHTML:''};ui.initializeStableDashboardElements=()=>({transactionEntry:entry,receiptEntry:receipt,lifecycle,tableBody:table,exportCsvButton:{}});ui.completeTransactionPageDiagnostic=()=>{};ui.transactionDiagnosticPageSource='fixture';
ui.renderTransactionRows([dto]);assert.ok(table.innerHTML.includes(dto.item));assert.ok(table.innerHTML.includes('Purchase (operational)'));assert.ok(table.innerHTML.includes('Non-financial'));assert.ok(table.innerHTML.includes('23.456'));assert.ok(!table.innerHTML.includes('transactions-row-action--correct'));
ui.renderTransactionDetail(ui.buildLocalTransactionDetail(dto));assert.ok(lifecycle.content.innerHTML.includes(dto.item));assert.match(lifecycle.description.innerText,/Excluded/);assert.match(lifecycle.state.innerText,/not verified/);assert.ok(!lifecycle.content.innerHTML.includes(dto.id));assert.doesNotMatch(lifecycle.content.innerHTML,/ING-|CONV-|COST_POOL_ALLOCATION/);
ui.fixtureDto=dto;
vm.runInContext("transactionsSearchIndex=[fixtureDto];transactionsSearchQuery='blue';transactionsLifecycleState='active';activeTransactionsTab='recent';activeTransactionDrilldown=null;",ui);
assert.equal(ui.filterTransactionsSearchIndex().length,1);vm.runInContext("activeTransactionDrilldown={type:'purchase'};",ui);assert.equal(ui.filterTransactionsSearchIndex().length,1);assert.equal(ui.filterTransactionDrilldown([dto],{type:'purchase'}).length,1);vm.runInContext("activeTransactionsTab='expenses';",ui);assert.equal(ui.filterTransactionsSearchIndex().length,0);
const n=calls.length;ui.prefetchVisibleTransactionDetails([dto]);assert.equal(calls.length,n);
console.log('PASS: operational business-label row/detail, acquisition cost, no technical identities or mutation actions, client search/drilldown, financial Expense exclusion, no canonical detail RPC and cost-only success message');
