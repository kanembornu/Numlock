...
- Sprint 5.13 Package 002 is complete locally. Before CSV quoting, the serializer inserts exactly one apostrophe before a first non-whitespace `=`, `+`, `-`, or `@`; valid negative numeric values in Qty or Amount columns and the existing `-` numeric placeholder remain unchanged. `testCsvExportContract()` now covers eight behavioral sanitizer cases and reports 14 scenarios while preserving the 25-entry runner, UTF-8 BOM, five visible columns, ordering, filename, Blob download, accessibility, and 72/2 query budget. A live unified 25/25 result remains required to close Sprint 5.13 acceptance.
