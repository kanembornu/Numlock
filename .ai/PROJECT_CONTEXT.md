...
`buildAnalyticsCache()` builds Aggregate Engine once. Migrated production builders consume cached aggregate outputs. Deterministic fixtures are authoritative for Summary and Revenue Trend and now cover Expense Breakdown pending live validation; the four remaining legacy builders are validation-only. Preserve the `getDashboardData()` response, Apps Script globals, formulas, spreadsheet reads, case-sensitive categories, date behavior, and frontend contract.
...
- The legacy Summary and Revenue Trend oracles are retired; four migration oracles remain until independent fixture coverage replaces them.
- The unified suite no longer depends on the Expense Breakdown oracle; its legacy chain remains pending retirement after live fixture validation.
