...
- `runAllBackendTests()` is the unified local/live backend gate and requires 34/34 PASS, including final UI stabilization, Logs visual, Settings visual, Transactions visual, Intelligence/Planning visual, Performance/Analytics visual, Dashboard Overview, tab framework, and UI shell/theme contracts plus the existing CSV-export, print-report, client-render performance, KPI target, unified business priority, period comparison, executive presentation, sparse-dataset resilience, date filtering, dashboard-state metadata, accessibility, responsive-shell, chart-presentation, frontend-dependency, reporting-metadata, scoped data-quality, and source invalid-date contracts.
...
- Active work is ordered as UI/UX 2.0 Packages 014–022. See `docs/UIUX-2.0-GAP-AUDIT.md` for navigation, visual gaps, missing skills, package sequence, and screenshot acceptance; see `docs/PROJECT-HISTORY.md` for granular sprint/package evidence.
