...
- Sprint 5.13 Package 001 is complete locally. One accessible Export CSV action downloads UTF-8 CSV through the browser Blob path using only the current filter's already rendered recent-transaction rows and visible table columns in displayed order. It excludes hidden spreadsheet data, backend/API changes, expanded history, drill-down, persistence, and GitHub Pages; `testCsvExportContract()` raises the ordered gate to 25/25. Upload, live Apps Script execution, and browser acceptance remain separate evidence until performed.
