...
- Sprint 5.15 Package 001 is complete as a documentation-only repository inventory. The source returns 37 top-level `getDashboardData()` fields: 31 are consumed by the production view and six are not directly rendered. Tests cover all fields across suites, but the sparse test's 36-entry required-property list omits `dateFilter`; its snapshot and dedicated tests cover that field separately. Print consumes rendered DOM, CSV consumes visible transaction-table cells, and drill-down depends on the seven-field, ten-row `recentTransactions` projection. External consumers, deployment access evidence, and field-level authorization remain unknown, so default-payload reduction is not ready.
