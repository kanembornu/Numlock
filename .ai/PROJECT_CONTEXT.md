...
- Sprint 5.13 Package 001 is committed at `563a7e4` and uploaded. One accessible Export CSV action downloads UTF-8 CSV through the browser Blob path using only the current filter's already rendered recent-transaction rows and visible table columns in displayed order; the stable DOM-query budget remains 72 ID queries and two selector queries with no response mutation. Closure is blocked because quote escaping does not neutralize leading spreadsheet-formula characters and `testCsvExportContract()` does not cover that requirement. The ordered runner contains 25 entries, but no post-fix unified 25/25 result is recorded; the latest fully verified gate remains Sprint 5.12's local 24/24.
- The single recommended next package is Sprint 5.13 Package 002: bounded CSV formula-injection hardening and acceptance closure. It adds neutralization and focused coverage without expanding exported fields, history, backend/API behavior, spreadsheet access, permissions, persistence, or UI scope, then requires the complete 25/25 gate.
