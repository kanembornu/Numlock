...
- The next recommended implementation package is a bounded first phase of P2-5: cache stable DOM references, avoid response-array mutation, and defer below-the-fold rendering while preserving the response contract and current stale-response behavior. API projection remains excluded until external consumers are inventoried.
