...
- The single recommended next package is Sprint 5.13 Package 001: a frontend-only CSV export of the current filter's already visible recent-transaction rows and columns. It is independently releasable and excludes hidden spreadsheet data, backend/API changes, expanded history, drill-down, and persistence.
